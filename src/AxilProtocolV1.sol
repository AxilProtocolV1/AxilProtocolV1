...
contract AxilProtocolV1 is EIP712, AccessControl, ReentrancyGuard, Pausable {
    ...
    //////////////////////////////////////////////////////////////*/
    function failedBurnQueue() external view returns (uint128) {
        return slotA.failedBurnQueue;
    }

    function totalBurned() external view returns (uint128) {
        return slotA.totalBurned;
    }

    function totalPendingRewards() external view returns (uint128) {
        return slotB.totalPendingRewards;
    }

    function lastBurnRetryBlock() external view returns (uint64) {
        return slotB.lastBurnRetryBlock;
    }
    ...
    event IntentSettled(
        uint128 indexed bucket,
        uint128 indexed mask,
    ...
    constructor(
        ...
        address _dexBroker,
        bytes32 _salt
    ) EIP712("AxilProtocolV1", "1") {
        if (
            _admin == address(0) || _signer == address(0) || _sweepReceiver == address(0)
                || _validatorPool == address(0) || _dexBroker == address(0)
        ) revert Axil__ZeroAddressNotAllowed();
        i_admin = _admin;
        i_signer = _signer;
        i_sweepReceiver = _sweepReceiver;
        i_validatorPool = _validatorPool;
        i_dexBroker = _dexBroker;
        i_domainSalt = _salt;
        ...
        _grantRole(TREASURY_ROLE, _sweepReceiver);
    ...
    function _minDeadlineBuffer() internal view returns (uint256) {
        return uint256(PACKED_CONSTANTS[0]);
    }

    function _feeBps() internal view returns (uint256) {
        return uint256(PACKED_CONSTANTS[1]);
    }

    function _vestingBlocks() internal view returns (uint64) {
        return uint64(PACKED_CONSTANTS[2]);
    }

    function _maxBatchSize() internal view returns (uint256) {
        return uint256(PACKED_CONSTANTS[3]);
    }
    ...
    function execute(
        ...
        if (config.useMerchantAllowlist > 0 && !hasRole(MERCHANT_ROLE, merchant)) {
            revert Axil__UnauthorizedAccess();
        }
    ...
    //////////////////////////////////////////////////////////////*/
    function autoBatchClaim(address[] calldata accounts, bytes32[] calldata intents, uint128 gasThreshold)
        external
    ...
    function autoRetryBurn(uint256 maxToBurn) external onlyRole(KEEPER_ROLE) nonReentrant {
        if (config.burnCooldownActive > 0) {
            if (
                block.number <= slotB.lastBurnRetryBlock
    ...
    function claimWithSignature(uint128 amount, bytes32 intentId, uint256 deadline, bytes calldata signature)
        external
        nonReentrant
        whenNotPaused
    {
        ...
        bytes32 hash = _hashTypedDataV4(keccak256(abi.encode(CLAIM_TYPEHASH, msg.sender, amount, intentId, deadline)));
    ...
    function _verifySignature(
        ...
        bytes32 hash = _hashTypedDataV4(
            keccak256(abi.encode(EXECUTE_TYPEHASH, merchant, user, packedIntent, amount, deadline, salt, msg.sender))
        );
    ...
    function getRewardVault(address account)
        external
        view
        returns (uint128 totalAmount, uint64 releaseBlock, uint64 lastClaimed, uint8 retryCount)
    {
        RewardVault storage vault = pendingRewards[account];
