...
abi.encode(
    ...
    function _executePayment(
        ...
        uint128 salt = uint128(uint256(keccak256(abi.encodePacked(
            block.timestamp,
            _merchant,
            _user,
            amount,
        ...
        bytes memory signature = _createExecuteSignature(
            _merchant,
            _user,
            intentId,
            amount,
...
uint256 amountUint = uint256(transferAmount);
        ...
        assertEq(
            user.balance,
            userBalBefore,
            "User's balance should not change (he only signs)"
        );
        ...
        assertEq(
            agent.balance,
            agentBalBefore - transferAmount,
            "Agent should spend exact amount"
        );
        console.log(" Agent spent:", transferAmount, "MON");

        // VERIFICATION 3: Merchant receives 99% instantly
        assertEq(
            merchant.balance - merchantBalBefore,
            expectedMerchant,
            "Merchant should receive 99% instantly"
        );
        console.log("\nINSTANT TRANSFERS:");
        ...
        assertEq(
            burnedAfter - burnedBefore,
            expectedBurn,
            "Burn should receive 0.2% instantly"
        );
        console.log("  Burn (0.2%):", expectedBurn, "MON");

        // VERIFICATION 5: Agent receives 0.2% pending rewards
        assertEq(
            agentRewardsAfter - agentRewardsBefore,
            expectedAgent,
            "Agent should receive 0.2% pending"
        );
        ...
        assertEq(
            userRewardsAfter - userRewardsBefore,
            expectedUser,
            "User should receive 0.2% cashback pending"
        );
...
// VERIFICATION 8: DEX Broker receives 0.2% pending rewards
        assertEq(
            dexRewardsAfter - dexRewardsBefore,
            expectedDex,
            "DEX Broker should receive 0.2% pending"
        );
